<x-guest-layout>
    <h2 class="text-2xl font-bold text-teal-800 text-center mb-6">Masuk ke SIPAKTA</h2>
    <p class="text-slate-500 text-sm text-center mb-6">Silakan masuk menggunakan email dan kata sandi Anda</p>
    ...
    <form method="POST" action="{{ route('login') }}">
        ...
        <div class="flex items-center justify-between mb-6">
            <label class="flex items-center">
                <input type="checkbox" name="remember" class="w-5 h-5 rounded border-slate-300 text-teal-600 focus:ring-teal-500">
                <span class="ml-2 text-sm font-bold text-slate-600">Ingat Saya</span>
            </label>
            @if (Route::has('password.request'))
                <a href="{{ route('password.request') }}" class="text-sm font-bold text-teal-600 hover:underline">Lupa Kata Sandi?</a>
            @endif
        </div>
        ...
        <p class="text-center text-sm text-slate-600 mt-5 font-semibold">
            Belum punya akun? <a href="{{ route('register') }}" class="text-teal-600 font-bold hover:underline">Daftar sekarang</a>
        </p>
    </form>
